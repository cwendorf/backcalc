...
backcalc_ratios <- function(ratio = NULL, se = NULL, n = NULL, df = NULL,
  ...
  if (is.null(ratio)) stop("A ratio (or two ratios) must be provided.")
  ...
  # Handle two ratios — log difference
  if (length(ratio) == 2) {
    estimate <- diff(log(ratio))
    
    # Handle SE as vector and df as vector (Welch approx)
  ...
  if (is.null(se)) stop("SE or CI must be provided or inferred for ratio inference.")
  ...
  names(result)[names(result) == "p_value"] <- ifelse(one_sided, "p-one", "p")
