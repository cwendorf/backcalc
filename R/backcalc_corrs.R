...
#' p-values, and test statistics from partial or varied combinations of summary and inferential statistics.
#'
...
#' \describe{
#'   \item{Estimnate}{Estimated correlation or difference in correlations.}
...
backcalc_corrs <- function(r = NULL, se = NULL, n = NULL, df = NULL,
                           p = NULL, ci = NULL, one_sided = FALSE, digits = 3) {
  ...
  if (length(estimate) == 2) {
    ...
    # If no se provided, try to infer it
    if (is.null(se)) {
      if (!is.null(n)) {
        se <- 1 / sqrt(n - 3)
  ...
  # Assemble result with rounding to decimal places
  result <- c(
